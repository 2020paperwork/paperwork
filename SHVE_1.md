...
主要调用了SHVE的函数和自己写的Graycode类，关键字的位图部分还没有写，不过位图也是以01的形式储存的，实际运行时加在地址信息产生的格雷后面就行。
...
public class Test {
    public static void main(String[] args) {
        ...
        long start,end;
        int []vector_1=test.transform(0.5,0.7);//测试数据
...
下面进行加密并记录加密时间
Enc Time: 545197500
选取vector_1和vector_2作为查询范围，查询的范围未化简前:
0 0 0 0 
0 0 0 1 
查询的范围化简后:
0 0 0 -1 
下面产生token并记录运行的时间
KeyGen Time: 2628800
Query Result of vector_1: true
Query Result of vector_2: true
Query Result of vector_3: false
Query Result of vector_4: false
Query Time: 822000
